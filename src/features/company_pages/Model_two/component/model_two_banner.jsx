...
const ModelTwoBanner = () => {
    return (
        <div>
            <div className="relative w-full h-[62rem] overflow-hidden">
                <svg
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 mt-[-5rem] flex w-[90%] z-10">
                    {/* Image (3/4 width) */}
                    <div className="w-3/4">
                        <img src={image} alt="Banner" className={`w-full h-[32rem] object-contain ${i18n.language == "ar" ? 'mr-[-8rem]' : 'ml-[-8rem]'}`} />
                    </div>
                    {/* Text (1/4 width) */}
                    <div className={`w-1/4 text-white flex flex-col justify-center mt-[-5rem] ${i18n.language == "ar" ? 'mr-[-11rem]' : 'ml-[-11rem]'}`}>
                        <p className="font-bold text-[1.8rem] lg:text-[2.3rem] ">
                            شـركة بناء الرافدين

                        </p>
                        <p className="text-[1.2rem] mt-[1rem] w-[140%]">
